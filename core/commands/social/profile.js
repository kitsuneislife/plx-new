...
const XYZ = {
  ...
  get wifeName() {
    return {
      X: 65 + this.wifeRect.X,
      Y: 10 + this.wifeRect.Y,
      W: 183,
      A: "left",
    };
  },
...
const init = async (Kanyon, message, args) => {
  try {
    message.runtime_internal = performance.now();
    ...
    // PROFILE FRAME
    if (message.content.split(/ +/).slice(1)[0] === "frame") {
      const ag = message.content.split(/ +/).slice(1)[1];
      const dData = await Kanyon.databaseResolveUser(message.author.id);
      ...
      function switchon() {
        DB.users
          ...
          .then((x) => null); // message.addReaction(':switchon:343511231434588161'));
      }
      ...
      function switchoff() {
        DB.users
          ...
          .then((x) => null); // message.addReaction(':switchoff:343511248085843968'));
      }
    ...
    let Target_Database = await Kanyon.databaseResolveUser(Target.id, Target);
    if (Target_Database.featuredMarriage?.length)
      Target_Database = Target_Database.populate("marriageData");
    ...
    const preprocessedParams = Promise.all([
      USERPROFILE.localData,
      USERPROFILE.globalRank,
      USERPROFILE.wifeData,
      USERPROFILE.commends,
    ]);
    ...
    try {
      ...
      img.ranks = Picto.getCanvas(
        `${paths.CDN}/build/profile/global-server-tag.png`,
      );
      img.defaultAvi = Picto.getCanvas(
        "https://cdn.discordapp.com/embed/avatars/0.png",
      );
      img.mainframe = Picto.getCanvas(
        `${paths.CDN}/build/profile/${Target.bot ? (PFLD ? "mainframe_bot" : "mainframe_bot") : "mainframe-nex-2"}.png`,
      );

      /*console.log(
        `${paths.CDN}/backdrops/${USERPROFILE.background}.png`,
        USERPROFILE.background,
      );*/

      img.background = Target.bot
        ? Picto.getCanvas(
            `${paths.CDN}/build/profile/${
      ...
      img.flair = Picto.getCanvas(
        `${paths.CDN}/flairs/${Target.bot ? "bot" : USERPROFILE.flair}.png`,
      ).catch((err) => Picto.getCanvas(`${paths.CDN}/flairs/default.png`));
      img.sticker =
        USERPROFILE.sticker &&
        Picto.getCanvas(`${paths.CDN}/stickers/${USERPROFILE.sticker}.png`);
      img.flag =
        USERPROFILE.countryFlag &&
        Picto.getCanvas(
          `${paths.CDN}/build/flags/${USERPROFILE.countryFlag}.png`,
        );
      img.aviFrame =
        USERPROFILE.profileFrame &&
        Picto.getCanvas(
          `${paths.CDN}/build/profile/frames/${USERPROFILE.profileFrame}.png`,
        );

      img.medals = USERPROFILE.medals.map(
        (mdl) =>
          new Object({
            canvas: Picto.getCanvas(`${paths.CDN}/medals/${mdl}.png`),
            index: Object.keys(USERPROFILE.medals).indexOf(mdl),
          }),
      );
      img.iconRubine = Picto.getCanvas(
        `https://cdn.discordapp.com/emojis/${_emoji("RBN").id}.png`,
      );
      img.iconSapphire = Picto.getCanvas(
        `https://cdn.discordapp.com/emojis/${_emoji("SPH").id}.png`,
      );
      img.global_roundel = Picto.XChart(
        120,
        USERPROFILE.percent,
        USERPROFILE.favColor,
        false,
        USERPROFILE.level,
      );
      ...
      if (!Target_Database) {
        USERPROFILE.tagline = "Not a Kanyon user";
        USERPROFILE.personalText = "This user does not play with Kanyon :c";
      }
      //= =========================================
      //                      Gather Graphic Text
      //= =========================================
      ...
      // @ts-ignore
      txt.name = await Picto.tagMoji(
        ctx,
        USERPROFILE.localName,
        `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px 'Panton Bold'`,
        TEXT[txt_type].COLOR,
      );

      txt_type = "TAGLINE";

      txt.tagline = Picto.tag(
        ctx,
        USERPROFILE.tagline,
      ...
      txt.persotex = Picto.block(
        ...
        XYZ.persotex.W,
        XYZ.persotex.H,
        { lineHeight: "20px", paddingY: 5 }, // 255, 70
      );

      txt_type = "RUBINES";

      txt.rubines = Picto.tag(
        ctx,
        miliarize(USERPROFILE.rubines),
        `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
        TEXT[txt_type].COLOR,
      );

      txt.sapphires = Picto.tag(
        ctx,
        miliarize(USERPROFILE.sapphires),
        `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
        TEXT[txt_type].COLOR,
      );
      ...
      txt.globalRank = Picto.tag(
        ctx,
        `#${miliarize(USERPROFILE.rank || 1)}`,
        `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
        TEXT[txt_type].COLOR,
      );

      txt.localRank = Picto.tag(
        ctx,
        `#${miliarize(USERPROFILE.localRank || 1)}`,
        `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
        TEXT[txt_type].COLOR,
      );
      ...
      if (isMarried) {
        ...
        img.wifeHeart = Picto.getCanvas(
          `${paths.CDN}/build/items/${USERPROFILE.wife.ring}.png`,
        );
      }
      if (isMarried) {
        txt_type = "WIFENAME";

        txt.wifeName = Picto.tag(
          ctx,
          USERPROFILE.wife.wifeName,
          `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
          TEXT[txt_type].COLOR,
        );
        ...
        txt.lovepoints = Picto.tag(
          ctx,
          `${USERPROFILE.wife.lovepoints} LVP`,
          `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
          TEXT[txt_type].COLOR,
        );

        txt.wifeSince = Picto.tag(
          ctx,
          USERPROFILE.wife.since,
          `${TEXT[txt_type].WEIGHT} ${TEXT[txt_type].SIZE}px '${TEXT[txt_type].FAMILY}'`,
          TEXT[txt_type].COLOR,
        );
      }
      ...
      const backdrop = new Promise(async (resolveAll) => {
        const backmost = new Promise(async (resolveBack) => {
          const canvas = Picto.new(800, 600);
          const ctx = canvas.getContext("2d");
          const [IMG, mainframe] = await Promise.all([
            img.background,
            img.mainframe,
          ]);
          const rad = {
        ...
        if (isMarried) {
          ...
          // @ts-expect-error NOTE tsc
          await img.wifeHeart.then((IMG) =>
            ctx.drawImage(IMG, wR.X + 6, wR.Y + 6, 55, 55),
          );
      ...
      const foreground = new Promise(async (resolveAll) => {
        ...
        const flair = img.flair.then((IMG) =>
          ctx.drawImage(
            IMG,
            XYZ.flair.X,
            XYZ.flair.Y,
        ...
        const rubine_n_roundel = (async () => {
          if (!Target.bot) {
            ctx.drawImage(
              txt.rubines.item,
              XYZ.rubines.X - txt.rubines.width,
              XYZ.rubines.Y + 3,
            );
            ctx.drawImage(
              txt.sapphires.item,
              XYZ.sapphires.X - txt.sapphires.width - txt.rubines.width - 50,
              XYZ.sapphires.Y + 3,
            );

            z = "global_roundel";

            const [imgRND, imgSPH, imgRBN] = await Promise.all([
              img[z],
              img.iconSapphire,
              img.iconRubine,
            ]);
            ...
            ctx.drawImage(imgRND, XYZ[z].X, XYZ[z].Y),
              ctx.drawImage(
                imgSPH,
                XYZ.sapphires.X + 5 - txt.rubines.width - 50,
                XYZ.sapphires.Y - 2,
                37,
                37,
              ),
      ...
      if (
        USERPROFILE.medalsArrangement &&
        USERPROFILE.medalsArrangement.valid.length > 0
      ) {
        ...
        if (valid_medals === 1 && valid.length >= 1) {
          const x1 = x + (150 / 2 - 50);
          const y1 = y + (150 / 2 - 50);

          await img.medals[valid[0]].canvas.then((IMG) =>
            ctx.drawImage(IMG, x1, y1, 150, 150),
          );
        } else if (valid_medals === 2 && valid.length >= 2) {
          const x1 = x;
          const y1 = y + 100;

          await Promise.all([
            img.medals[valid[0]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x1, y1, 100, 100),
            ),
            img.medals[valid[1]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x1 + 100, y1, 100, 100),
            ),
          ]);
        } else if (valid_medals === 3 && valid.length >= 3) {
          const x1 = x;
          const y1 = y;

          await Promise.all([
            img.medals[valid[0]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x1 + (200 / 2 - 50), y1, 100, 100),
            ),
            img.medals[valid[1]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x1, y1 + 100, 100, 100),
            ),
            img.medals[valid[2]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x1 + 100, y1 + 100, 100, 100),
            ),
          ]);
        } else if (valid_medals === 4 && valid.length >= 4) {
          await Promise.all([
            img.medals[valid[0]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x, y, 100, 100),
            ),
            img.medals[valid[1]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x + 100, y, 100, 100),
            ),
            img.medals[valid[2]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x, y + 100, 100, 100),
            ),
            img.medals[valid[3]].canvas.then((IMG) =>
              ctx.drawImage(IMG, x + 100, y + 100, 100, 100),
            ),
          ]);
        } else {
          // Caso geral para mais de 4 medals ou menos de 1 medal
          let ind = 0;
          let row = 0;
          let col = 0;

          await Promise.all(
            valid.map((index) => img.medals[index].canvas),
          ).then((medals) => {
            while (ind < valid.length) {
              if (img.medals[ind]) {
                ctx.drawImage(medals[ind], x + 68 * col, y + 68 * row, 64, 64);
              }
              ind += 1;
              col += 1;
              if (col >= 3) {
      ...
      await Picto.getCanvas(`${paths.CDN}/build/profile/litostar.png`).then(
        (IMG) => {
      ...
      if (isMarried) {
        ["wifeName", "lovepoints", "wifeSince"].forEach(
          (/** @type {keyof XYZ} */ z) => {
            Picto.setAndDraw(
              ctx,
              txt[z],
        ...
        if (!Target.bot || ["name", "tagline"].includes(z)) {
          if (z == "name") {
            ...
            Picto.setAndDraw(
              ...
              txt[z],
              XYZ[z].X,
              XYZ[z].Y + (z.includes("Rank") && !isMarried ? 80 : 0),
              XYZ[z].W,
              XYZ[z].A,
            );
            ctx.globalCompositeOperation = "destination-over";
      ...
      const hexes = new Promise(async (resolve) => {
        ...
        ctx3.drawImage(
          await img.hex_pic,
          XYZ.avatar.X + XYZ.offset_hex,
          XYZ.avatar.Y + XYZ.offset_hex,
        );
      ...
      return Promise.all([backdrop, foreground, hexes]).then(async (arr) => {
        ...
        try {
          if (Target_Database) {
            ...
            if (Target_Database.switches?.badges?.template == "artist")
              bottomTag = "artist";
            ...
            if (bottomTag) {
              const tierframe = await Picto.getCanvas(
                `${paths.BUILD}profile/bottomtags/${bottomTag}.png`,
              );
              ctx.drawImage(tierframe, 160 + 248, 565);
            }

            if (
              bottomTag === "translator" &&
              Target_Database.switches.translator
            ) {
              const flag = await Picto.getCanvas(
                `${paths.BUILD}flags/${Target_Database.switches.translator}.png`,
              );
              ctx.drawImage(flag, 160 + 313, 573, 32, 21);
            }

            if (
              Target_Database.blacklisted &&
              Target_Database.blacklisted != ""
            ) {
              const bliste = await Picto.getCanvas(
                `${paths.CDN}/build/bliste.png`,
              );
        ...
        return await FINALIZE(message, canvas);
      });
      //});
    } catch (e) {
    ...
    options: [
      {
        name: "player",
        description:
          "The person to display their Profile, leave blank for self",
        type: 6,
        required: false,
      },
    ],
...
async function FINALIZE(message, canvas) {
  ...
  if (message.content.includes("-ni")) noimg = true;

  messageToSend += message.content.includes("-bm")
    ? `${performance.now() - message.runtime_internal}ms  (${postBuffer.toFixed(3)}ms de Buffer)\n`
    : "";

  if (message.fake) return ["", new MessageAttachment(buff, "profile.png")];
  ...
  if (message.content.includes("-bm"))
    messageToSend = `${noimg ? "**No-IMG**" : ""} \`⏱️${((performance.now() - message.runtime_internal) / 1000).toFixed(3)}s\``;
}
