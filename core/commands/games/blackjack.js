...
const fetchCard = (card, deck) => {
  if (typeof card === "string" && card.startsWith("JOKER")) {
    return Picto.getCanvas(
      `${paths.CDN}/build/cards/casino/JOKERS/${card.split("-")[1] || deck || "default"}.png`,
    );
  }
  if (card[0] === "X")
    return Picto.getCanvas(
      `${paths.CDN}/build/cards/casino/${deck || "default"}/backface.png`,
    );

  const { rank, suit } = cardValue(card);
  return Picto.getCanvas(
    `${paths.CDN}/build/cards/casino/${deck || "default"}/${suit}${rank}.png`,
  );
};
...
const renderHand = async (HANDS, deck, bjd, current) => {
  ...
  if (HANDS.length === 1) {
    const handImages = await Promise.all(
      HANDS[0].map((card) => fetchCard(card, deck)),
    );
    ...
    HANDS.forEach(async (hand, i) => {
      const handImages = await Promise.all(
        hand.map((card) => fetchCard(card, deck)),
      );
...
const drawTable = async (PL, DL, DATA_A, DATA_B, drawOpts) => {
  ...
  const fel = await Picto.getCanvas(`${_ASSETS}/feltro.png`);
  const insurGlow = await Picto.getCanvas(`${_ASSETS}/ins_avail.png`);
  const insurEnabled = await Picto.getCanvas(`${_ASSETS}/ins_using.png`);
  const dio = await Picto.getCanvas(`${_ASSETS}/dio.png`);
  const bWin = await Picto.getCanvas(`${_ASSETS}/BLACKJACK-win.png`);
  const bLose = await Picto.getCanvas(`${_ASSETS}/BLACKJACK-lost.png`);
  const bJoker = await Picto.getCanvas(`${_ASSETS}/JOKER-win.png`);
  const stando = await Picto.getCanvas(`${paths.BUILD}STANDO.png`);
  const alphamask = await Picto.getCanvas(`${_ASSETS}/alphamask.png`);

  //console.log("akiii");

  const chip = await Picto.getCanvas(`${_ASSETS}/chips-${chips}.png`);
  const chipI = insurance
    ? await Picto.getCanvas(`${_ASSETS}/chips-${Math.ceil(chips / 2)}.png`)
    : null;
  const you = drawOpts.enemyStando
    ? dio
    : await Picto.getCanvas(Kanyon.user.avatarURL({ extension: "png" }));
  const me = await Picto.getCanvas(msg.author.avatarURL({ extension: "png" }));
  const bjk = bjkWIN ? bWin : bjkLOSE ? bLose : jkrWIN ? bJoker : null;
  const joker = jkrWIN
    ? await Picto.getCanvas(
        `${paths.Build}cards/casino/JOKERS/${SCORE_A.split("-")[1] || "default"}.png`,
      )
    : null;

  const betImg = Picto.tag(
    c,
    miliarize(bet),
    "900 italic 40px 'Panton Black'",
    "#e6d084",
  );
  const betTxt = Picto.tag(
    c,
    _v.bet.toUpperCase(),
    "600 30px 'Panton'",
    "#4a8b45",
  ...
  const insImg = Picto.tag(
    c,
    miliarize(insurance * Math.ceil(basebet / 2)),
    "600 italic 36px 'Panton Black'",
    "#2ab099",
  );
  ...
  const numP = Picto.tag(
    c,
    SCORE_A,
    "900 italic 36px 'Panton Black',Sans",
    "#fff",
  );
  c.drawImage(numP.item, 648 - numP.width, 305);

  const numD = Picto.tag(
    c,
    SCORE_B,
    "900 italic 36px 'Panton Black',Sans",
    "#fff",
  ...
  const BUSTED = Picto.tag(
    c,
    `${_v.BUST.toUpperCase()}!`,
    "900 italic 40px 'Panton Black'",
    "#ea2e2e",
    { style: "#2b2b3b", line: 10 },
  );
  ...
  if (
    ...
    drawOpts.actions.forEach((action, i) => {
      ...
      const button = Picto.tag(
        c,
        action,
        "900 italic 16pt 'Panton Black'",
        actionColor,
      ).item;
      const xpos = (i % 3) * 155;
      const ypos = ~~(i / 3) * 40;
...
const DECK = async (msg, args) => {
  ...
  if (["default", "vegas", "reset"].includes(msg.args[1])) {
    ...
    return msg.channel.send(
      `${rand$t($t.responses.verbose.interjections.acknowledged)} ` +
        `${$t.games.blackjack.switchdeck} ${rand$t($t.responses.verbose.opinion_decks)}`,
    );
  }
  ...
  if (targetDeck && USERDATA.modules.skinInventory.includes(targetDeck.id)) {
    await DB.users.set(msg.author.id, {
      "modules.skins.blackjack": targetDeck.localizer,
    });
    P.deckname = `${_emoji("plxcards").no_space}\`${targetDeck.name}\``;
    let deckSwitchMessage =
      `${rand$t($t.responses.verbose.interjections.acknowledged)} ${$t.games.blackjack.switchdeck}` +
      `${rand$t($t.responses.verbose.opinion_decks)}`;
    if (randomize(1, 6) === 3 && $t.games.blackjack.switchdeckEgg.length > 1) {
      deckSwitchMessage = `${rand$t($t.responses.verbose.interjections.acknowledged)} ${$t.games.blackjack.switchdeckEgg} ${_emoji("plxOof")}`;
    }
...
function testDoubleDown(balance, bet, currentHand) {
  return balance >= bet * 2 && currentHand.length === 2;
}

function testSplit(balance, bet, currentHand) {
  return (
    balance >= bet * 2 &&
    Blackjack.handValue([currentHand[0]]) ===
      Blackjack.handValue([currentHand[1]]) &&
    currentHand.length === 2
  );
}

function hitStandMessage(P, canDoubleDown, canSplit, canInsurance) {
  const HIT_TXT = $t.games.blackjack.hit;
  const DOUBLE_TXT = $t.games.blackjack.double;
  const SPLIT_TXT = $t.games.blackjack.split;
  const PASS_TXT = $t.games.blackjack.pass;

  const hitstand = `${
    !canDoubleDown && !canSplit
      ? HIT_TXT + PASS_TXT
      : `${HIT_TXT} ${canDoubleDown ? DOUBLE_TXT : ""}${
          canSplit ? SPLIT_TXT : ""
        }${PASS_TXT}`
  }
${$t.games.blackjack.surrender_helper}
${canInsurance ? $t.games.blackjack.insurance_helper : ""}
`;
  return hitstand;
}

async function getFinalHand(
  blackjack,
  playerHand,
  dealerHand,
  deck,
  ...
  async function ProcessHand(currentHand) {
    ...
    if (
      typeof currentHandValue === "string" &&
      currentHandValue.startsWith("JOKER")
    ) {
      nextHand();
      return ProcessHand(currentHand);
      // continue;
    ...
    const canInsurance = testInsurance(
      balance,
      totalBet,
      currentHand,
      dealerHand,
    ...
    const [POLLUX_HAND_GFX, PLAYER_HAND_GFX] = await promiseTimeout(
      3000,
      Promise.all([
        renderHand(hands, deck, bjkD, currentHand),
        renderHand([visibleHand], deck, bjkP),
    ...
    if (errored)
      Promise.reject(new Error(`Error during checks => \n${POLLUX_HAND_GFX}`));
    ...
    const scenario = await drawTable(
      PLAYER_HAND_GFX,
      POLLUX_HAND_GFX,
      USR_HAND,
      POL_HAND,
    ...
    if (currentHand.insuredLastTurn) {
      await msg.channel
        .send(
          `${_emoji("plxbjkinsu")} **Insurance has been placed for this match!**`,
          {
            file: await scenario.toBuffer("image/png", imageOptions),
            name: "blackjack.png",
            description: `Blackjack Round ${1 + (log?.length || 0)}. Kanyon has ${POL_HAND.status + " " + POL_HAND.val}, ${msg.author.username} has ${USR_HAND.status + " " + USR_HAND.val}`,
          },
        )
        .then(async (mm) => {
    ...
    const buttonsToAdd = {
      ...
      row2: [canInsurance ? BUTTONS.insurance : undefined, BUTTONS.surrender],
    };
    ...
    const responses = await Promise.race([
      msg.channel
        .awaitMessages(
          ...
          {
            filter: (fm) => fm.author.id === msg.author.id,
            max: 3,
            time: 30_000,
            errors: ["time"],
          },
        )
        .then(async (collected) => console.log(collected)),
      scene_msg
    ...
    if (responses?.length === 0)
      return log.push(`> ${_emoji("TIME2")} Player stand **${USR_HAND.val}**`);
    const action =
      responses || responses[0]?.content?.toLowerCase() || responses[0]?.id;
    ...
    if (action === "insurance") {
      ...
      log.push(
        `> ${_emoji("plxbjkinsu")} \`INSURANCE\` (${Math.ceil(bet / 2)})`,
      );
    }
    if (action === "surrender") {
      ...
      log.push(
        `> ${_emoji("plxbjksurr")} Player \`SURRENDER\` @ **${USR_HAND.val}**`,
      );
      if (currentHand === hands[hands.length - 1])
    ...
    if (action === "stando") {
      ...
      msg.channel.send($t.eastereggs.konodioda);
    }
    if (
      action === "stando" ||
      action === "stand" ||
      Blackjack.handValue(currentHand) >= 21
    ) {
    ...
    if (action === "hit") {
      ...
      log.push(
        `> ${newVal > 21 ? _emoji("plxbjkbust") : _emoji("plxcards")} Player \`HIT\` **${USR_HAND.val}** > **${newVal}**`,
      );
    }
    if (action === "split" && canSplit) {
      ...
      log.push(
        `> ${_emoji("plxcards")} Player \`SPLIT\` **${USR_HAND.val / 2}s**`,
      );
...
function gameResult(playerValue, dealerValue) {
  ...
  if (playerValue.toString().includes("JOKER") || playerValue > dealerValue)
    return "win";

  return "loss";
}

const init = async (Kanyon, msg, args) => {
  const USERDATA = await Kanyon.databaseResolveUser(msg.author.id);
  ...
  const polluxNick = "Kanyon";
  const playerName = msg.member.nick || msg.author.username;

  const P = { lngs: msg.lang };
  const v = {};
  v.ONGOING = $t.games.blackjack.ongoing;
  v.NEWGAME = $t.games.blackjack.newgame;
  v.RESULT = $t.games.blackjack.result;
  v.BUST = $t.games.blackjack.bust;

  v.HAND = $t.games.blackjack.hand;

  v.HIT = $t.games.blackjack.hit;
  v.STAND = $t.games.blackjack.stand;
  v.SPLIT = $t.games.blackjack.split;
  v.DOUBLE_DOWN = $t.games.blackjack.double;

  v._WIN = $t.games.blackjack.youwin;
  v._LOSE = $t.games.blackjack.youlose;
  v._JOKER = $t.games.blackjack.joker;
  v._EVEN = $t.games.blackjack.even;
  v._PRIZE = $t.responses.generic.plus_rubines;
  v._ANTIPRIZE = $t.responses.generic.minus_rubines;

  v.bet = $t.ts.dict.bet;

  v.insu = $t.ts.$.insuBet; // , { lngs: msg.lang, number: 25 });
  v.nofunds = $t.ts.$.noFundsBet; //", { lngs: msg.lang, number: USERDATA.modules.RBN });
  v.insuFloor = $t.ts.$.insuFloor; // ", { lngs: msg.lang, number: 25 });
  v.ceiling = $t.games.ceilingBet.replace("%emj%", _emoji("RBN")); //", { lngs: msg.lang, number: MAX_BET }).replace("%emj%", _emoji("rubine"));

  if (await Blackjack.gameExists(msg.author.id)) {
  ...
  const bet = Math.abs(parseInt(args[1]));

  if (Number.isNaN(bet))
    return (
      msg.reply("Eii, vc precisa apostar amor") ||
      msg.command.invalidUsageMessage(msg)
    );
  ...
  try {
    ...
    const dealerHand = blackjack
      .getHand()
      .map((card) =>
        card.startsWith("JOKER") ? `${randomize(1, 10)}H` : card,
      );
    ...
    const hitstandPre = hitStandMessage(
      P,
      canDoubleDown,
      canSplit,
      canInsurance,
    ...
    return msg.channel.send(v.NEWGAME).then(async (introMessage) => {
      ...
      const log = {
        ...
        usr: [],
        Kanyon: [
          `> ${_emoji("plxcards")} Kanyon \`OPEN\` **${Blackjack.handValue(dealerHand)}**`,
        ],
      };
      ...
      if (
        playerHandValueCalc !== "Blackjack" &&
        !playerHandValueCalc.toString().includes("JOKER")
      ) {
        playerHands = await getFinalHand(
          blackjack,
          playerHand,
          dealerHand,
          myDeck,
        ...
        while (
          (Blackjack.isSoft(dealerHand) ||
            Blackjack.handValue(dealerHand) < 17) &&
          !noHit
        ) {
          log.Kanyon.push(
            `> ${
              Blackjack.handValue(dealerHand) > 21
                ? _emoji("plxbjkbust")
                : _emoji("plxcards")
            } Kanyon \`HIT\` **${Blackjack.handValue(dealerHand)}** ` +
              `> **${Blackjack.handValue(blackjack.hit(dealerHand, { nojoker: true }))}**`,
          );
        }
        if (noHit) {
          log.Kanyon.push(`> ${_emoji("plxbjkwin")} **Kanyon wins**`);
        }

        log.embed.description = `Game finished in ${log.Kanyon.length + log.usr.length} turns.`;
        introMessage.delete();
      } else {
      ...
      playerHands.forEach((hand, i) => {
        ...
        let lossOrGain = Math.floor(
          (["loss", "bust", "surrender"].includes(result)
            ? -1
            : result === "push"
              ? 0
              : 1) *
        ...
        lossOrGain +=
          dealerValue === "Blackjack" ? insuranceAmount : -insuranceAmount;
        if (dealerValue === "Blackjack")
          log.Kanyon.push(
            `> ${_emoji("plxbjkbjk")} **Kanyon has a Blackjack**`,
          );
        if (playerValue === "Blackjack")
          log.usr.push(`> ${_emoji("plxbjkbjk")} **Player has a Blackjack**`);
        if (playerValue.toString().includes("JOKER"))
          log.usr.push(`> ${_emoji("plxbjkjkr")} **Player has a Joker**`);
        ...
        H_DATA.result =
          playerHands.length === 1
            ...
            : ` ${result.replace(/(^\w|\s\w)/g, (ma) => ma.toUpperCase())}${
                result !== "push" ? `, ${lossOrGain}` : `, ${"Rubines"}"} back`
              }\n`;
        ...
        splitExplain.push(
          `${_emoji("plxcards").no_space}\`\u200b${`${i + 1}`.padStart(2, " ")}\` : **\`\u200b${`${lossOrGain}`.padStart(6, " ")}\`** ` +
            `${_emoji("RBN")} ${RESULT_EMOJI(result)}${
              hand.doubled ? _emoji("plxbjk2x") : ""
      ...
      if (Blackjack.handValue(playerHands[0]).toString().includes("JOKER"))
        PLAY_RES = v._JOKER;
      ...
      if (finalResult === "surrender")
        PLAY_RES = $t.games.blackjack.surrender_res;

      if (dealerValue === "Blackjack")
        PLAY_RES = $t.games.blackjack.blackjack_lose;

      if (
        playerHands.length === 1 &&
        Blackjack.handValue(playerHand) === "Blackjack"
      )
        PLAY_RES = $t.games.blackjack.blackjack_win;

      // TOP -> Commentary

      if (
        playerHand.doubled &&
        ["loss", "bust", "surrender"].includes(finalResult)
      )
        PLAY_RES += `\n${$t.games.blackjack.double_lose}`;
      else if (playerHand.doubled)
        PLAY_RES += `\n${$t.games.blackjack.double_win}`;

      if (playerHand.insurance && dealerValue === "Blackjack")
        PLAY_RES += `\n${$t.games.blackjack.insurance_win}`;
      else if (playerHand.insurance)
        PLAY_RES += `\n${$t.games.blackjack.insurance_lost}`;
      ...
      if (winnings !== 0) {
        if (winnings > 0) {
          if (!splitWins) {
            ...
            /*Progression.emit("play.blackjack.windouble", {
                msg,
                userID: msg.author.id,
              });*/
            /*Progression.emit("streak.blackjack.win", {
              value: 1,
              msg,
              userID: msg.author.id,
            });*/
            /*Progression.emit("play.blackjack.win", {
              msg,
              userID: msg.author.id,
            });*/
          }
      ...
      if (splitWins) {
        ...
        /*Progression.emit("play.blackjack.winsplit", {
          value: splitWins,
          msg,
          userID: msg.author.id,
        });*/
      ...
      const rebalance = resp
        ?.replace("%R%", _emoji("RBN") + Math.abs(winnings))
        .replace("$t(keywords.RBN_plural)", $t.keywords.RBN_plural);

      blackjack.endGame();

      let eeebuffer;
      try {
        eeebuffer = await scenario.toBuffer("image/png", imageOptions);
      } catch (err) {
        eeebuffer = scenario.toBuffer();
      }
      msg.channel
        .send({
          content: PLAY_RES,
          files: [
            {
              attachment: eeebuffer,
              name: "blackjack.png",
              description: `Blackjack Final Round.`,
            },
          ],
        })
        .then((m) => {
          if (splitExplain.length) {
            log.embed.fields.push({
              name: `**${splitExplain.length > 1 ? $t.games.blackjack.splitbreak : $t.games.blackjack.result}**`,
              value: splitExplain.join("\n"),
              inline: false,
            });
            log.embed.fields[1] = {
              name: "Kanyon Turn",
              value: log.Kanyon.join("\n"),
              inline: true,
            };
            log.embed.fields[0] = {
              name: "Player Turn",
          ...
          m.channel
            .send({
              content: rebalance,
              embeds: [splitExplain.length ? log.embed : {}],
            })
            .catch(() => null);
        });
      ...
      if (hasJoker) {
        gameJokers.forEach((JKR, i) => {
          msg.channel.send(
            `\`${JKR}\` eff -- immediate -- J:${i}/${gameJokers.length}`,
          );
        });
      }
    });
...
module.exports = {
  ...
  // hooks,
  aliases: ["bjk"],
  teleSubs: [{ label: "decks", path: "inventory/decks", pass: "casino" }],
  autoSubs: [
    {
      label: "deck",
      gen: DECK,
      options: {
        ...
        invalidUsageMessage: (msg) => {
          Kanyon.autoHelper("force", { msg, cmd: "decks", opt: "cosmetics" });
        },
      },
    },
  ],
