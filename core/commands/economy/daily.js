...
//   const P = { prefix: Kanyon.prefix };
//   const v = {
//     last: $t.interface.daily.lastdly,
//     next: $t.interface.daily.next,
//     streakcurr: $t.interface.daily.streakcurr,
//     expirestr: $t.interface.daily.expirestr,
//   };

//   if (msg.args[0] === "info") {
//     msg.args[0] = "status";
//     msg.channel.send(
//       "*`INFO` is deprecated, please use `STATUS` to check remaining time*",
//     );
//   }

//   const [userData, dailyPLXMember] = await Promise.all([
//     Kanyon.databaseResolveUser(msg.author.id),
//     Kanyon.resolveMember(Premium.OFFICIAL_GUILD, msg.author.id).catch(
//       () => msg.author,
//     ),
//   ]);
...
//       _emoji("nope") + $t.responses.daily.dailyNope,
//     ); // FIXME[epic=bsian] Remove the "in" in the locale
//   }
...
//         ctx.drawImage(await constantAssets[`soft${softStreak}`], 0, 0),
//       );
//     })
//     .once("isRoadTo50-isNot50", () => {
//       processQueue.push(async () =>
//         ctx.drawImage(await constantAssets.prev30, 0, 0),
//       );
//     })
//     .once("isRoadTo100-isNot100", () => {
//       processQueue.push(async () =>
//         ctx.drawImage(await constantAssets.prev100, 0, 0),
//       );
//     })
//     .once("is30", () => {
//       processQueue.push(async () => {
...
//     .once("guildBooster", () => {
//       processQueue.push(async () =>
//         ctx.drawImage(await constantAssets.boost, 0 - 50, 0),
//       );
//     })
//     .once("userDonator", (donoBoost) => {
//       processQueue.push(async () => {
//         const donoE = Picto.getCanvas(
//           `${paths.CDN}/images/donate/icony/${userData.donator}-small.png`,
//         );
//         const numberDONOBOOST = Picto.tag(
//           ctx,
//           `+ ${donoBoost}`,
//           "italic 900 38px 'Panton Black'",
//           "#FFF",
//           { line: 6, style: "#223" },
//         );
//         const textDONOBOOST = Picto.tag(
//           ctx,
//           userData.donator?.toUpperCase() || "UNKNOWN",
//           "italic 900 15px 'Panton Black'",
//           "#FFF",
//         );

//         const [donoTag, donoEmblem] = await Promise.all([
//           constantAssets.donoT,
//           donoE,
//         ]);
...
//           60,
//           40,
//           "italic 900 45px 'Panton Black'",
//           "#FFF",
//           300,
//           { style: "#1b1b25", line: 12 },
//         );
//         ctx.restore();

//         const textStreak = Picto.tag(
//           ctx,
//           "STREAK ",
//           "italic 900 14px 'Panton Black'",
//           "#FFF",
//         ); // ,{line: 6, style: "#223"} )
//         const textEXP = Picto.tag(
//           ctx,
//           "EXP",
//           "italic 900 18px 'Panton Black'",
//           "#FFF",
//           { line: 6, style: "#223" },
//         );
//         const numberStreak = Picto.tag(
//           ctx,
//           timedUsage.userDaily.streak,
//           "italic 900 32px 'Panton Black'",
//           "#FFF",
//         ); // ,{line: 6, style: "#223"} )
//         const numberStreakPrize = Picto.tag(
//           ctx,
//           myDaily.EXP,
//           "italic 900 38px 'Panton Black'",
//           "#FFF",
//           { line: 6, style: "#223" },
//         );
//         const numberBoostPrize = Picto.tag(
//           ctx,
//           `+${miliarize(myDaily.PSM)}`,
//           "italic 900 35px 'Panton Black'",
//           "#FFF",
//           { line: 6, style: "#223" },
//         );
...
//         if (timedUsage.userDaily.insured)
//           ctx.drawImage(await constantAssets.expTagInsu, 0, 0);
//         else if (timedUsage.streakStatus === "recovered") {
//           ctx.drawImage(await constantAssets.expTagWARNING, 0, 0);
//           Picto.popOutTxt(
//             ctx,
//             "Streak insurance activated!", //$t("Streak insurance activated!", P)
//             360,
//             540,
//             "italic 900 35px 'Panton Black'",
//             "#FFF",
...
//           Picto.popOutTxt(
//             ctx,
//             "Streak lost!",  //$t("Streak Lost!", P),
//             22,
//             506,
//             "italic 900 42px 'Panton Black'",
//             "#FFF",
//             400,
//             { line: 8, style: "#F23" },
//           );
...
//           ctx.drawImage(
//             numberStreakPrize.item,
//             160 - numberStreakPrize.width,
//             530,
//           );
...
//       itemName = $t.item.cosmo_fragment.name;
...
//     if (P.count)
//       fields[index].value += `${itemoji} **${P.count}** ${itemName}\n`;
...
//     P.insuCount =
//       userData.modules?.inventory?.find((i) => i.id === "keepstreak")?.count ||
//       0;
...
//       userData.modules?.inventory?.find((i) => i.id === "keepstreak")?.count ||
//       0;
//     postmortem = $t.responses.daily.insuranceConsumed
//   }
//   if (timedUsage.streakStatus === "lost") {
//     if (timedUsage.userDaily.lastStreak <= 1) {
//       P.insuCount =
//         userData.modules?.inventory?.find((i) => i.id === "keepstreak")
//           ?.count || 0;
//       postmortem = $t.responses.daily.firstDaily
//     } else {
//       P.oldStreak = timedUsage.userDaily.lastStreak;
//       const streakfixes =
//         userData.modules?.inventory?.find((i) => i.id === "streakfix")?.count ||
//         0;
//       postmortem = `${$t.responses.daily.streakLost}${streakfixes ? $t.responses.daily.yesRestorerInfo : $t.responses.daily.noRestorerInfo}`;
//     }
//   }
...
//         content: `**Aqui estão suas recompensas amor~!` /*`**\nCheck also \`${msg.prefix}errands\` for daily progression tasks and get more bonuses!`*/,
//         embed: {
//           description: `☕ ${rand$t($t.responses.daily.dailyBonus)}`,
//           fields,
//           timestamp: new Date(),
//           footer: {
//             icon_url: msg.author.avatarURL,
//             text: `${msg.author.tag}\u2002•  ${bar}`,
//           },
//           color: 0x03dffc,
...
//         files: [
//           {
//             attachment: await dailyCard.toBuffer("image/png"),
//             name: "daily.png",
