function CanvasTextWrapper(canvas, text, options) {
  ...
  //console.log(opts.font, "entry font from opts");
  //console.log(context.font, "entry font from context");
  ...
  let fontSize = opts.font.match(/\d+(px|em|%)/g)
  ...
  //console.log({ fontParts, fontSize, original_font: context.font });
  ...
  function checkLength(words) {
    ...
    words.forEach(function (word, index) {
      ...
      if (tokenLen > MAX_TXT_WIDTH) {
         let k = 0;
      while ( context.measureText(testString + word[k]).width <= MAX_TXT_WIDTH && k < word.length) {
        k++
            testString += word[k];
        }
  ...
  function drawText() {
    let skipLineOnMatch = multiNewLineDelimiter + " ";
    for (let i = 0; i < lines.length; i++) {
